...
-- Topic: Scalar SQL Functions (Part 1 of 2)
-- Scope: Setup script + Part A (String Functions)
-- Continued in Lab 11: Part B (Numeric & Date/Time functions)
-- ============================================================
...
-- Customers (extra spaces, mixed case, NULLs - on purpose)
...
-- Task A1: Trimmed name alongside original
SELECT CustID, CustName AS Original, TRIM(CustName) AS CleanedName
FROM Customer;

-- Task A2: UPPERCASE and lowercase versions of each name
SELECT CustID, UPPER(CustName) AS UpperName, LOWER(CustName) AS LowerName
FROM Customer;

-- Task A3: Trimmed name and its character count
SELECT CustID, TRIM(CustName) AS CleanedName, CHAR_LENGTH(TRIM(CustName)) AS NameLength
FROM Customer;
...
SELECT CustID, LEFT(CustName, 3) AS ShortName
FROM Customer;

-- Task A8: Mask phone numbers - first 4 chars, then 'XXX-XXXX'
SELECT CustName, Phone, CONCAT(LEFT(Phone, 4), 'XXX-XXXX') AS MaskedPhone
FROM Customer
...
SELECT ProdID, LPAD(ProdID, 5, '0') AS PaddedID
FROM Product;

-- Task A11: Product names containing 'Pro', with position of 'Pro'
...
SELECT CustID,
       CASE
           WHEN LOCATE(' ', TRIM(CustName)) = 0 THEN TRIM(CustName)
           ELSE SUBSTRING(TRIM(CustName), 1, LOCATE(' ', TRIM(CustName)) - 1)
       END AS FirstName
FROM Customer;
