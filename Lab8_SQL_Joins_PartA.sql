...
-- Topic: SQL Joins (Part 1 of 2)
-- Scope: Setup script + Part A (INNER, LEFT, RIGHT joins)
-- Continued in Lab 9: Part B (Self joins, multi-table joins, combined)
-- ============================================================
...
-- Task A2: Same as A1, but keep employees with NULL DeptID (LEFT JOIN)
SELECT e.EmpID, e.EmpName, d.DeptName, d.Location
FROM Employee e
...
-- Task A4: Every project with department name and location
...
-- Task A5: Employees not assigned to any project (anti-join pattern)
SELECT e.EmpID, e.EmpName
FROM Employee e
LEFT JOIN Assignment a ON e.EmpID = a.EmpID
WHERE a.EmpID IS NULL;

-- Task A6: Projects that currently have no assignments
SELECT p.ProjectID, p.ProjectName
FROM Project p
LEFT JOIN Assignment a ON p.ProjectID = a.ProjectID
WHERE a.ProjectID IS NULL;

-- Task A7: Employees in Engineering with salary, sorted descending
...
-- Task A9: Every department and count of employees (include zero-employee depts)
SELECT d.DeptName, COUNT(e.EmpID) AS EmployeeCount
FROM Department d
LEFT JOIN Employee e ON e.DeptID = d.DeptID
GROUP BY d.DeptID, d.DeptName;

-- Task A10: FULL OUTER JOIN of Employee and Department, via UNION
SELECT e.EmpName, d.DeptName
FROM Employee e
LEFT JOIN Department d ON e.DeptID = d.DeptID
UNION
